...
import { LoadingSkeleton, TableSkeleton } from "@/components/loading-skeleton";
...
export default function DashboardPage() {
    ...
    const loadData = async () => {
      try {
        ...
        try {
          ...
          console.warn("Could not fetch interviews from API:", error);
  ...
  const avgScore =
    interviews.length > 0
      ...
      : 0;
  const highestScore = interviews.length > 0
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div className="flex-1 space-y-2">
                        ...
                        <p className="font-semibold line-clamp-2">
                          {interview.question || "Question not available"}
                        </p>
                        <p className="text-sm text-muted-foreground line-clamp-2">
                          {interview.answer || "Answer not available"}
                        </p>
                      </div>
                      {interview.score && (
        ...
        {interviews.length > 0 && (
          <SlideUp delay={0.6}>
            <Card className="p-8 mt-8 bg-linear-to-br from-primary/10 to-accent/10 border-primary/20 text-center space-y-4">
              <h3 className="text-xl font-semibold">Ready For Another Challenge?</h3>
              <p className="text-muted-foreground max-w-md mx-auto">
                Keep practicing with different roles and levels to improve your interview skills.
              </p>
              <Link href="/start">
                <Button className="gap-2">
                  <PlayCircle className="w-4 h-4" />
